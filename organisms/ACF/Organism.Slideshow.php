...
class ACF_Slideshow extends OrganismTemplate {
	...
	public function getMarkup() {
		...
		foreach ( $this->slides as $slide_index => $slide_data ) {
			self::generateSlide( $slide_index, $slide_data );
		}
	...
	public function generateSlide( $slide_index, $slide_data ) {
		...
		$slide_args = [
			...
			'structure'  => [
				'background' => [
					// Background attributes are set from slides data in generateSlides method
					'attributes' => [
				...
				'text'       => [
					'parts' => [
						'title'       => [
							'tag'     => 'h2',
							// Content is set from slides data in generateSlides method
							'content' => ''
						],
						'subtitle'    => [
							'tag'     => 'h3',
						...
						'link'        => [
							'atom'    => 'Link',
							// Content & href are set from slides data in generateSlides method
							'href'    => '',
							'content' => ''
						]
					]
	...
	private function parseSlideshowOptionsAsAttribute() {
		...
		$acf_slideshow_settings_json = json_encode( $slideshow_vars );
